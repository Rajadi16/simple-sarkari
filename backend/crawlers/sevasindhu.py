"""
Seva Sindhu Karnataka adapter — sevasindhu.karnataka.gov.in

Person 1 (Aditya) — Ingestion & Source Verification

Seva Sindhu is Karnataka's service delivery portal. It publishes
circulars and notices using a NIC template. PDFs linked from the homepage.
"""
...
class SevaSindhuAdapter(BaseCrawlerAdapter):
    """Adapter for Seva Sindhu Karnataka portal."""

    PARSER_NAME = "sevasindhu_karnataka_v1"
    ...
    async def fetch_listing(self) -> list[CandidateDocument]:
        ...
        seed_urls = self.source.get("seed_urls", ["https://sevasindhu.karnataka.gov.in/"])
        ...
        for seed_url in seed_urls:
            ...
            for a in soup.find_all("a", href=True):
                ...
                h = urlparse(abs_url).hostname or ""
                is_pdf = href.lower().endswith(".pdf")
                is_notice = any(k in href.lower() for k in
                                ["circular", "notice", "order", "notification", "go-", "gazette"])
                if not (is_pdf or is_notice):
                    continue
                if h not in (base_host, "www." + base_host):
                    # Allow .gov.in sub-domains linked from the portal
                    if not h.endswith(".gov.in") and not h.endswith(".kar.nic.in"):
                        continue
                ...
                candidates.append(CandidateDocument(
                    ...
                    department="Karnataka e-Governance Centre",
                    language="en-IN",
    ...
    async def parse(self, candidate: CandidateDocument, result: FetchResult) -> CanonicalCircular:
        ...
        title = ext.get("title") or candidate.title or "Seva Sindhu Notice"
        ...
        return CanonicalCircular(
            ...
            source=SourceInfo(
                ...
                source_domain="sevasindhu.karnataka.gov.in",
