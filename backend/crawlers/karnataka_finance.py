...
Publishes budget documents, GOs, circulars, and treasury orders as PDFs.
SSL note: NIC CA — verify=False applied.
"""
...
_LISTING_URLS = [
    "https://finance.karnataka.gov.in/page/Government-Orders/en",
    "https://finance.karnataka.gov.in/page/Circulars/en",
    "https://finance.karnataka.gov.in/page/Budget/en",
]
...
def _infer_doc_type(text: str, url: str) -> str:
    ...
    if "go " in t or "government order" in t or "/go" in t:
...
class KarnatakaFinanceAdapter(BaseCrawlerAdapter):
    ...
    PARSER_NAME = "karnataka_finance_v1"
    ...
    async def parse(self, candidate: CandidateDocument, result: FetchResult) -> CanonicalCircular:
        ...
        title = ext.get("title") or candidate.title or "Karnataka Finance Document"
        ...
        return CanonicalCircular(
            ...
            source=SourceInfo(
                source_id="karnataka_finance",
                source_name="Karnataka Finance Department",
                source_domain="finance.karnataka.gov.in",
